...
TEST_CASE("def_encoding_traits_consistency")
{
    ...
    REQUIRE(def_encoding_traits::alphabet() == expected_alphabet);
    REQUIRE(def_encoding_traits::alphabet_size() == 64);
    REQUIRE(def_encoding_traits::has_pad());
    REQUIRE(def_encoding_traits::pad() == uint8_t{ '=' });
    REQUIRE(def_encoding_traits::invalid_index() == 64);

    uint32_t pos = 0;
    bool wrong_pos = false;

    for (const char symbol : def_encoding_traits::alphabet())
    {
        pos = def_encoding_traits::index_of(static_cast<uint8_t>(symbol));
        if (pos == def_encoding_traits::invalid_index())
            break;

        if (def_encoding_traits::char_at(pos) != static_cast<uint8_t>(symbol))
        {
            wrong_pos = true;
            break;
        }
...
using encoding_with_pad = base64::custom_encoding_traits<alt_alphabet, '='>;
...
using encoding_without_pad = base64::custom_encoding_traits<url_alphabet, 0>;
